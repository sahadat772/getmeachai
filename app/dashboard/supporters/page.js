...
export default async function SupportersPage() {
  ...
  return (
    <div style={{ padding: '2rem 1.5rem', maxWidth: '72rem', margin: '0 auto' }}>
      ...
      {/* Summary cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }} className="sm:grid-cols-3">
        {[
          ...
          { icon: '☕', label: 'মোট ট্রানজেকশন', value: transactions.length + ' টি', color: 'var(--accent)' },
        ].map(function (card, i) {
          return (
            <div key={card.label} style={{ padding: '1.25rem', borderRadius: '18px', background: 'var(--surface)', border: '1px solid var(--border)', animation: 'fadeInUp 0.5s ease both', animationDelay: (i * 0.1) + 's', transition: 'all 0.2s' }}
      ...
      {supporters.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {/* Full table */}
          <div style={{ borderRadius: '20px', background: 'var(--surface)', border: '1px solid var(--border)', overflow: 'hidden', animation: 'fadeInUp 0.5s ease 0.2s both' }}>
                  ...
                  {supporters.map(function (s, i) {
                    return (
                      <tr key={s.name} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.15s' }}
                        ...
                        <td style={{ padding: '14px 20px', color: 'var(--text-faint)', fontSize: '13px' }}>
                          {i < 3 ? medals[i] : i + 1}
                        </td>
            ...
            {/* Mobile cards */}
            <div className="md:hidden" style={{ display: 'flex', flexDirection: 'column', gap: '0', padding: '0' }}>
              {supporters.map(function (s, i) {
                return (
                  <div key={s.name} style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '0.75rem', transition: 'background 0.15s' }}
                    onMouseEnter={function (e) { e.currentTarget.style.background = 'var(--surface-2)'; }}
                    onMouseLeave={function (e) { e.currentTarget.style.background = 'transparent'; }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'var(--gradient)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: i < 3 ? '1.2rem' : '13px', fontWeight: 700, color: 'white', flexShrink: 0, fontFamily: 'var(--font-heading)' }}>
                      {i < 3 ? medals[i] : i + 1}
                    </div>
